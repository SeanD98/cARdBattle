...
public class DemonBattleSceneTrackedImageManager : MonoBehaviour
{
    public static DemonBattleSceneTrackedImageManager instance;
    [SerializeField]
    public static GameObject[] arObjectsToPlace;
    ...
    void Awake()
    {
        Analytics.CustomEvent("Battle - Demons - started");
    ...
    void Update()
    {
        if (Input.touchCount > 0)
        {
            ...
            if (touch.phase == TouchPhase.Began)
            {
                ...
                {
                    if (hitObject.transform.name == "GoblinFighter")
                    {
                        //Fight Button etc
                        CornerText.text = "Corner One Selected";
                    }
                    if (hitObject.transform.name == "EnemyFighter")
                    {
