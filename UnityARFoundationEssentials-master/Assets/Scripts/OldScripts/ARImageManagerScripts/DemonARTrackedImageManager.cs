...
public class DemonARTrackedImageManager : MonoBehaviour
{
    [SerializeField]
    ...
    void Update()
    {
        if (Input.touchCount > 0)
        {
            ...
            if (touch.phase == TouchPhase.Stationary)
            {
                ...
                {
                    //NAMES NEED CHANGED TO MATCH NEW MODELS
                    if (hitObject.transform.name == "GolblinAROne")
                    {
                        SceneManager.LoadScene("DemonBattleScene");
                    }
                    if (hitObject.transform.name == "GoblinARTwo")
                    {
                        SceneManager.LoadScene("DemonBattleScene");
                    }
                    if (hitObject.transform.name == "GoblinARThree")
                    {
                        SceneManager.LoadScene("DemonBattleScene");
                    }
                    if (hitObject.transform.name == "GoblinARTwoFour")
                    {
                        SceneManager.LoadScene("DemonBattleScene");
                    }
