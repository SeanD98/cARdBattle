...
public class MonsterARTrackedImageManager : MonoBehaviour
{
    [SerializeField]
        ...
        if (Input.touchCount > 0)
        {
            ...
            if (touch.phase == TouchPhase.Stationary)
            {
                ...
                if (Physics.Raycast(ray, out hitObject))
                {
                    ...
                    {
                        //firstTap = true;
                        arTappedText.text = "AROne selected";
                        SceneManager.LoadScene("BattleField");
                        //tapToConfirm();
                    }
                    if (hitObject.transform.name == "ARTwo")
                    {
                        arTappedText.text = "ARTwo tapped!";
                    }
                    if (hitObject.transform.name == "ARThree")
                    {
                        arTappedText.text = "ARThree tapped!";
                    }
                    if (hitObject.transform.name == "ARFour")
                    {
                        arTappedText.text = "ARFour tapped!";
                    }
